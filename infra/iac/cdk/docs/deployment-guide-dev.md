...
1. アプリケーションの作成
   1. [parameter.ts](../parameter.ts)の`systemParameter.baseDomainName`の値を確認
   ...
   3. 新規アプリケーションを作成
      ...
      - ホームページ URLを設定：
        ...
        ```
        https://dev.{systemParameter.baseDomainName}
        ```

        例: systemParameter.baseDomainName が example.com の場合
      ...
      - リダイレクトURLを設定：
        ...
        ```
        https://dev.{systemParameter.baseDomainName}/api/auth/callback/typetalk
        ```
...
1. parameter.tsの変更をコミット
   以下の設定値の変更をコミットする:
   - systemParameter.baseDomainName
...
共有アカウントで作成したホストゾーンのNSレコードを、ドメインレジストラに設定する。
この設定により、全環境のドメイン名解決が有効になる。
...
1. ヘルスチェックエンドポイントの確認
   1. parameter.tsの`systemParameter.baseDomainName`で設定したドメインに対して以下を実行

      ```bash
      curl https://api.dev.DOMAIN_NAME/healthcheck
      ```
...
1. ブラウザでの動作確認
   1. アクセス確認
      - URL: https://dev.DOMAIN_NAME にアクセス
